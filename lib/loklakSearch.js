...
module.exports = function (bot, redisClient, loklakRedis, loklak) {
    ...
    return loklak.getTweets(searchTerm)
      .then(function (tweets) {
        if (tweets) {
          ...
          sendTweet(msg, tweets.pop(), false, 0, tweets.length + 1)
  ...
  // returns a promise
  function sendTweet(msg, tweet, replyKeyboard=false, current, count=current) {
    return new Promise(function (resolve, reject) {
      var formattedTweet = tweetMessageFormatter(tweet, current, count)
      var options = {}
      if (replyKeyboard) {
        var reply_markup = {
      ...
      bot.sendMessage(msg.chat.id, formattedTweet, options)
    })
  }
  ...
  function pushTweets (msg, tweets) {
    if (tweets) {
      loklakRedis.pushTweets(msg, tweets)
        .then((result) => {
  ...
  function nextTweet(msg) {
    return new Promise(function (resolve, reject) {
      loklakRedis.popTweet(msg.from)
        .then(function (tweet) {
          if (tweet) {
            sendTweet(msg, JSON.parse(tweet), true, tweet.count)
            resolve(tweet)
          }
          else {
            resolve()
