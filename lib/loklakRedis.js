...
module.exports = function (redisClient) {
  ...
  function sessionCommand(user, command) {
    return new Promise(function (resolve, reject) {
      redisClient.set(`user:sessionCommand:${user.id}`, command, function (err, result) {
        if (err) {
          reject(err)
        }
        else {
  ...
  function getSessionCommand(user) {
    return new Promise(function (resolve, reject) {
      redisClient.get(`user:sessionCommand:${user.id}`, function (err, result) {
        if (err) {
          reject(err)
        }
        else {
