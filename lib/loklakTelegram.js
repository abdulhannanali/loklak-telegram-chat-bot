...
module.exports = function (bot, redisClient) {
  ...
  Twitter profile link: 🔗 https://twitter.com/${screen_name}`
            ...
            bot.sendMessage(msg.chat.id, userMessage, {})
              .then(function () {
                bot.sendPhoto(msg.chat.id, request(userInfo.user.profile_image_url), {
                  ...
                  // reply_markup: {
                  //   keyboard: [["/user !getlocation"]],
                  //   one_time_keyboard: true,
                  //   resize_keyboard: true
                  // }
                })
                if (userInfo.user.location_point) {
                  bot.sendLocation(msg.chat.id, userInfo.user.location_point[1], userInfo.user.location_point[0], {
                    reply_to_message_id: msg.message_id
                  })
                }
              })
      ...
      bot.sessionCommand(msg.from, "/user")
        .then(function (result) {
          bot.sendMessage(msg.chat.id, "Nice! Now enter the username you want to know about")
        })
        .catch(function (error) {
          bot.sendMessage(msg.chat.id, "Hey There! /user is used for getting more information about the 🙎 users 🙎 on twitter. You type the user after the /user.\nFormat: /user [username]\nExample: /user fossasia")
        })
    }
  })
