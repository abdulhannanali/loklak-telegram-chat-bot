...
module.exports = function (bot, redisClient) {
  ...
  bot.onText(/\/search(.*)/, function (msg, match) {
    ...
    if (query) {
      ...
      bot.sendMessage(msg.chat.id, "Sorry no query found! Type /help for help regarding the searching of tweets")
    }
  })

  bot.onText(/\/browse\s*next\s*[➡️]*\s*/g, function (msg, match) {
    loklakRedis.popTweet(msg.from)
      .then(function (tweet) {
        if (tweet) {
          bot.sendMessage(msg.chat.id, tweet, {
            reply_markup: replyKeyboard(false)
          })
        }
        else {
  ...
  bot.on('message', function (msg) {
    var definedEndpoints = [
      ...
      /\/search.*/,
      /\/user.*/,
      /\/browse\s*next\s*[➡️]*\s*/g,
      /\/browse\s*back\s*[⬅️]*\s*/g
    ]
